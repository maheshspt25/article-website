...
export default async function TravelDetailCatPage({ params }: ArticlePageProps) {
  const article = await getArticleBySlug(params.slug);
  if (!article) {
    notFound();
  }

  const relatedArticles = await getLatestArticles('travel', 4);
  ...
  return (
    <>
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
        ...
        <RelatedContent title="Related Travel Guides" articles={relatedArticles} />
      </div>
    </>
  );
}
